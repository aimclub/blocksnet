...
class Blocks_model:
    ...
    def _get_city_geometry(self) -> gpd.GeoDataFrame:
        ...
        city_geometry = (
            gpd.GeoDataFrame.from_features(city_geometry["features"]).set_crs(self.global_crs).to_crs(self.city_crs)
        )
    ...
    def _get_water_bounds(self) -> gpd.GeoDataFrame:
        ...
        # Check if all geometries are valid
        water_bounds.loc[
            water_bounds.geometry.geom_type.isin(["LineString", "MultiLineString"]), "geometry"
        ] = water_bounds.geometry.buffer(self.WATER_WIDTH)
    ...
    def _fill_deadends(self, city_geometry: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
        ...
        city_geometry.geometry = city_geometry.geometry.map(
            lambda road: road.buffer((self.ROAD_WIDTH + 1)).buffer(-(self.ROAD_WIDTH + 1))
        )
