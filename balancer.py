...
def kindergarten_area(kids):
    if kids >= 280:
        return kindergarten_area_ranges(280) + kindergarten_area(kids - 280)
    else:
...
def school_area(schoolkids):
    if schoolkids >= 1100:
        return school_area_ranges(1100) + school_area(schoolkids - 1100)
    else:
...
class MasterPlan:
    ...
    def sc_area(self, population):
        return school_area(self.SC_coef * population)

    def kg_area(self, population):
        return kindergarten_area(self.KG_coef * population)
    ...
    def fun(self, x):
        return self.area - self.living_area(x[0], x[1]) - self.sc_area(x[0] + self.current_unprov_schoolkids) \
               - self.kg_area(x[0] + self.current_unprov_kids) \
               - self.green_area(x[0] + self.current_unprov_green_population, x[1]) - self.op_area(x[0]) \
               - self.parking1_area(x[0]) - self.parking2_area(x[0])

    def bnds_and_cons(self):
        self.cons = ({'type': 'ineq', 'fun': lambda x: self.max_population - x[0]},
                     {'type': 'ineq',
                      'fun': lambda x: self.max_living_area - self.living_area(x[0], x[1]) - self.parking1_area(x[0])},
                     {'type': 'ineq',
                      'fun': lambda x: self.max_free_area - self.green_area(x[0] + self.current_unprov_green_population,
                                                                            x[2]) - self.op_area(x[0])},
                     {'type': 'ineq',
                      'fun': lambda x: self.max_industrial_area - self.sc_area(x[0] + self.current_unprov_schoolkids)
                                       - self.kg_area(x[0] + self.current_unprov_kids) - self.parking2_area(x[0])},
    ...
    def optimal_values(self, population, b, G):
        ...
        return {'area': self.area,
                ...
                'b': b * Hectare,
                'green_coef': G * Hectare,
                'living_area': self.living_area(population, b) + self.current_living_area,
                'sc_area': self.sc_area(population+self.current_unprov_schoolkids),
                'kg_area': self.kg_area(population+self.current_unprov_kids),
                'green_area': self.green_area(population+self.current_unprov_green_population, G) + self.current_green_area,
