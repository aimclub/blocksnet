...
class NetworkModel:
    ...
    def best_location_full(self, func_type):
        ...
        if func_type == 'Спортивное':
            prov = 'provision_sp'
        for n in self.graph:
            if self.graph.nodes[n]['is_zone'] == True and self.graph.nodes[n]['type'] == 'потенциальное' and self.graph.nodes[n][
                'people_around'] * 2 / self.graph.nodes[n]['area'] <= 1:
                neighbor = list(self.graph.neighbors(n))
                for neigh in neighbor:
                    if self.graph.nodes[neigh][prov] == 0:
                        #sum += 1 / self.graph[n][neigh]['time']
                        sum += self.graph.nodes[n]['people_around'] / self.graph[n][neigh]['time']
                if sum > max:
                    index = n
                    max = sum
                sum = 0
    ...
    def best_location1(self, func_type, area_type):
        ...
        for n in self.graph:
            if self.graph.nodes[n]['is_zone'] == True and self.graph.nodes[n]['type'] == 'потенциальное' and self.graph.nodes[n][
                'people_around'] * 2 / self.graph.nodes[n]['area'] <= 1 and self.graph.nodes[n]['area_type'] == area_type:
                neighbor = list(self.graph.neighbors(n))
                for neigh in neighbor:
                    if self.graph.nodes[neigh][prov] == 0:
    ...
    def local_search(self):
        ...
        func = {'Многофункциональное': 'provision_mf', 'Событийное': 'provision_ev', 'Спортивное': 'provision_sp',
        ...
        while i < len(nodes):
            if self.graph.nodes[nodes[i]]['is_zone'] == True and self.graph.nodes[nodes[i]]['type'] == 'существующее' and nodes[
                i] not in self.existed:
                ls -= 1
                clsd[nodes[i]] -= 1
                ...
                while True:
                    ...
                    if loc[0] != 0:
                        ...
                        break
                for n in self.graph:
                    if self.graph.nodes[n]['is_zone'] == True and self.graph.nodes[n]['type'] == 'существующее' and \
                            self.graph.nodes[n]['functional_type'] == closed_func and n not in self.existed:
    ...
    def get_houses(self):
        ...
        for n in self.graph:
            if self.graph.nodes[n]['is_zone'] == False:
                ...
                self.house['provision_ev'][indx] = self.graph.nodes[n]['provision_ev']
                self.house['provision_mf'][indx] = self.graph.nodes[n]['provision_mf']
                self.house['provision_ed'][indx] = self.graph.nodes[n]['provision_ed']
                self.house['provision_sp'][indx] = self.graph.nodes[n]['provision_sp']
                self.house['provision'][indx] = self.graph.nodes[n]['provision']
        return self.house
