...
def _impute_non_living_area(blocks_df: pd.DataFrame):
    if "non_living_area" not in blocks_df.columns:
        ...
        if "living_area" in blocks_df.columns and "build_floor_area" in blocks_df.columns:
            blocks_df["non_living_area"] = blocks_df["build_floor_area"] - blocks_df["living_area"]


def _impute_missing_columns(func):
    @wraps(func)
    def wrapper(blocks_df: pd.DataFrame, *args, **kwargs):
        blocks_df = blocks_df.copy()
        _impute_non_living_area(blocks_df)
        return func(blocks_df, *args, **kwargs)

    return wrapper


@_impute_missing_columns
def calculate_density_indicators(blocks_df: pd.DataFrame) -> pd.DataFrame:
    ...
    blocks_df = blocks_df.assign(
        **{
            ...
            SHARE_BUSINESS_COLUMN: blocks_df.non_living_area / blocks_df.footprint_area,
