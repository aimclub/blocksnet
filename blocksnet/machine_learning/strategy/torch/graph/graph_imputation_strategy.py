...
class TorchGraphImputationStrategy(TorchBaseStrategy):
    ...
    def _create_dataloader(
        self, x: torch.Tensor, y: torch.Tensor, edge_index: torch.Tensor, batch_size: int, use_subgraphs: bool
    ) -> DataLoader:
        """
    ...
    def _epoch_test(
        self, x_test: torch.Tensor, y_test: torch.Tensor, edge_index: torch.Tensor, criterion: torch.nn.Module
    ) -> Tuple[float, float]:
        """
    ...
    def train(
        ...
        self._model = self._initialize_model(input_dim, output_dim)
        ...
        # Create DataLoader
        dataloader = self._create_dataloader(x_train_tensor, y_train_tensor, edge_index, batch_size, use_subgraphs)
        ...
        pbar = tqdm(
            range(epochs),
            disable=log_config.disable_tqdm,
